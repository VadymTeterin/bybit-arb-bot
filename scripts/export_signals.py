...
def export_signals(
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["timestamp", "symbol", "spot_price", "futures_price", "basis_pct", "volume_24h_usd"])
    ...
    if keep is not None and keep > 0:
        ...
        all_csv = sorted(out_path.parent.glob(f"{prefix}_*.csv"), key=lambda p: p.stat().st_mtime, reverse=True)
...
def main() -> None:
    ...
    p.add_argument("--out", type=str, default=None, help="Output CSV path (e.g., exports/signals_YYYY-MM-DD_HHMM.csv)")
    p.add_argument("--last-hours", type=int, default=24, help="Export only last N hours (default: 24)")
    p.add_argument("--since", type=str, default=None, help="Export from UTC ISO time (e.g., 2025-08-10T00:00:00)")
    ...
    p.add_argument("--tz", type=str, default=None, help='Localize timestamp to "Europe/Kyiv" or offset like "+03:00"')
    p.add_argument("--keep", type=int, default=None, help="Keep only last N CSV files with the same prefix")
    args = p.parse_args()
