...
def test_launcher_cmd_structure():
    ...
    assert r"python.exe" in content.lower(), "launcher should run some python executable"
...
def test_launcher_cmd_executes_in_its_own_dir(tmp_path: Path):
    ...
    # Гнучкий патерн: ЗАМІНИТИ ЛЮБИЙ РЯДОК, де є 'python.exe' (незалежно від регістру/шляху)
    pattern = re.compile(r'^[^\r\n]*python\.exe[^\r\n]*$', flags=re.MULTILINE | re.IGNORECASE)
    assert pattern.search(original), "Test could not find a line with python.exe in launcher_export.cmd"

    # Надійна підміна: створити ran.txt і щось записати у export.log
    ...
    assert ran_marker.exists(), "launcher should create logs\\ran.txt via the substitute command"
